...
def load_data(fname, stage, start, end):
    ...
    bgraph = m.fragments.fgraphs["base"]
    for i in bgraph.get_vertices():
        if bgraph.vp.type[i] == "ph":
            neigh = [bgraph.vp.type[n] for n in bgraph.get_all_neighbors(i)]
            if "pyr" in neigh:
                bgraph.vp.type[i] = "phr"
    m.fragments.merge_frags("base", "post", ["pyr", "phr"], "rotor")
...
if __name__ == "__main__":
    ...
    dip, cell = load_data("../../4-nvt_e0/A_methanol_100.mfp5", "nvt_e0", 0, 500000)
    ...
    for i in fields:
        dip, cell = load_data(f"e_{i}/A_methanol_100.mfp5", "nvt_e", 0, 100000)
        dip = np.array(list(dip.values())).sum(axis=0)
        p = dip/V
        p_diff = p-p_nf
